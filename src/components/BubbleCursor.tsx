...
const BubbleCursor = () => {
  ...
  const updateMousePosition = useCallback((e: MouseEvent) => {
    ...
    const now = Date.now();
    if (now - lastTrailUpdate.current > 50) { // Update every 50ms instead of every mousemove
      setTrail(prev => [
    ...
    <div className="fixed inset-0 pointer-events-none z-[100] mix-blend-difference">
      ...
      <motion.div
        className={`fixed w-6 h-6 rounded-full ${
          theme === 'dark' 
            ? (isHovering ? 'bg-white' : 'bg-tech-purple') 
            : (isHovering ? 'bg-black' : 'bg-tech-blue')
      ...
      {trail.map((point, index) => (
        <motion.div
          ...
          className={`fixed w-2 h-2 rounded-full ${
            theme === 'dark' ? 'bg-tech-purple/40' : 'bg-tech-blue/40'
          }`}
          style={{
            left: point.x - 4,
            top: point.y - 4,
