...
const DigitalGlobe = () => {
  ...
  // Updated country data with all countries showing Syrian flag
  const countries: CountryData[] = [
    { name: 'Syria', flag: '🇸🇾', lat: 34.8021, lng: 38.9968, capital: 'Damascus' },
    { name: 'Germany', flag: '🇸🇾', lat: 51.1657, lng: 10.4515, capital: 'Berlin' },
    { name: 'Finland', flag: '🇸🇾', lat: 61.9241, lng: 25.7482, capital: 'Helsinki' },
    { name: 'UAE', flag: '🇸🇾', lat: 23.4241, lng: 53.8478, capital: 'Abu Dhabi' },
    { name: 'Saudi Arabia', flag: '🇸🇾', lat: 23.8859, lng: 45.0792, capital: 'Riyadh' },
    { name: 'Malaysia', flag: '🇸🇾', lat: 4.2105, lng: 101.9758, capital: 'Kuala Lumpur' },
    { name: 'France', flag: '🇸🇾', lat: 46.2276, lng: 2.2137, capital: 'Paris' },
    { name: 'Jordan', flag: '🇸🇾', lat: 30.5852, lng: 36.2384, capital: 'Amman' },
    { name: 'Japan', flag: '🇸🇾', lat: 36.2048, lng: 138.2529, capital: 'Tokyo' },
    { name: 'Turkey', flag: '🇸🇾', lat: 38.9637, lng: 35.2433, capital: 'Ankara' },
    { name: 'Iraq', flag: '🇸🇾', lat: 33.2232, lng: 43.6793, capital: 'Baghdad' }
  ];
  ...
  useEffect(() => {
    ...
    const initThreeJS = async () => {
      ...
      globeRef.current = { globeGroup, connectionPoints, countryMarkers, dataFlows };
      ...
      const animate = () => {
        ...
        globeGroup.children.forEach((child, index) => {
          ...
          if ('material' in child && child.material && typeof child.material === 'object') {
            ...
            if ('color' in material && material.color instanceof THREE.Color) {
              ...
              if (isConnection) {
                ...
                material.opacity = baseOpacity + Math.sin(time * 1.5 + index * 0.3) * 0.2;
              }
            }
          }
        });
  ...
  useEffect(() => {
    const handleResize = () => {
      if (rendererRef.current && mountRef.current) {
        ...
        if (sceneRef.current) {
          const camera = sceneRef.current.children.find((child: any) => child.type === 'PerspectiveCamera');
          if (camera) {
            camera.aspect = 1;
        ...
        >
          <div className="bg-secondary/95 backdrop-blur-sm border border-tech-purple/30 rounded-lg p-3 shadow-lg">
            <div className="flex items-center gap-2 mb-1">
              <span className="text-2xl" style={{ fontSize: '1.5rem', lineHeight: '1' }}>
                {hoveredCountry.flag}
              </span>
