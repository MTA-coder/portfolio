...
import { Calendar, MapPin, ExternalLink, Building, Flag, Zap, TrendingUp } from 'lucide-react';
...
const StorytellingTimeline: React.FC<StorytellingTimelineProps> = ({ items }) => {
  ...
  return (
    <div className="timeline-container relative" ref={timelineRef}>
      ...
      <div className={cn(
        ...
        isMobile ? "left-6 transform-none" : "left-1/2 transform -translate-x-1/2"
      )}>
        <motion.div 
  ...
  return (
    ...
    >
      ...
      <div className={cn(
        ...
        isMobile ? "left-6 transform -translate-x-1/2" : "left-1/2 transform -translate-x-1/2"
      )}>
        <motion.div
          className={cn(
            "absolute inset-0 rounded-full border-2 bg-background shadow-xl",
            isActive ? "border-tech-purple ring-4 ring-tech-purple/20" : "border-tech-purple/60"
          )}
          animate={{
      ...
      <motion.div 
        className={cn(
          ...
          isMobile 
            ? "ml-20 mr-4" 
            : (isRight ? "w-[45%] ml-auto mr-0" : "w-[45%] mr-auto ml-0")
        )}
        whileHover={{ 
          scale: 1.02,
        ...
        animate={{
          ...
          boxShadow: isActive ? "0 20px 40px rgba(168, 85, 247, 0.3)" : "0 10px 25px rgba(0, 0, 0, 0.1)"
        }}
      >
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h4 className="font-bold text-lg text-foreground">{item.company}</h4>
        ...
        <div className="mb-6">
          ...
          <h3 className="text-xl font-bold mb-3 leading-tight text-foreground">{item.title}</h3>
        ...
        <ul className="space-y-3">
          {item.description.map((desc, i) => (
            ...
            >
              ...
              <span className="leading-relaxed group-hover:text-foreground transition-colors">{desc}</span>
            </motion.li>
          ))}
        </ul>
